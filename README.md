...
Run **Developer: Reload Window**, then open **Customize** and confirm that the twelve skills are listed. Teams and Enterprise organizations can import `germanfndez/fiveai-skills` from **Dashboard > Plugins > Team Marketplaces**. Update a local clone with `git pull --ff-only`; uninstall it by removing that local plugin directory.
