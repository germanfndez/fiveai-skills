...
Use `SendNUIMessage` (Lua) or `SEND_NUI_MESSAGE` (other languages) to send data to your UI:
...
```csharp
// C# example (requires Newtonsoft.Json)
...
Control focus with `SET_NUI_FOCUS` / `SetNUIFocus`:
